...
<body>
	...
	<div id="wrap" align="center">
		...
		<table class="list">
			<tr>
				<c:if test="${member.lev == 'A'}">
					<td colspan="8" style="border: white; text-align: right"><input
						type="button" value="도서 등록" id="regibtn"
						onclick="location.href='BookServlet?command=book_register_form'">
					</td>
				</c:if>
			</tr>
			<c:forEach var="book" items="${booklist}">
				<%-- <tr class="record" style="border-bottom: 2px solid #8c8ec8;">
					<td>${book.num}</td>
					<td>${book.category}</td>
				...
				<tr class="record">
					...
					<input type="hidden" name="pictureurl" value="${book.pictureurl}"></td>
					<td>
					...
					<br> <b>제목</b>
						&nbsp; <a href="BookServlet?command=book_detail&num=${book.num}">
							${book.title} </a> <input type="hidden" name="title" value="${book.title}">
