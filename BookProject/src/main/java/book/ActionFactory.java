...
import cart.CartCheckFormAction;
...
public class ActionFactory { // 페이지 이동을 위한 팩토리 (커맨드 패턴으로 작업 처리를 위한 명령 처리 클래스 필요함)
	...
	public Action getAction(String command) {
		...
		} else if(command.equals("user_manager_form")) {
			action = new UserManagerFormAction();
		} else if(command.equals("user_manager")) {
			action = new UserManagerAction();
		...
		} else if(command.equals("cart_check_form")) {
			action = new CartCheckFormAction();
